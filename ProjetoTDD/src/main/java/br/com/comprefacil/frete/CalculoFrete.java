...
public class CalculoFrete {
	...
	public Frete calcularFrete(String peso, BigDecimal largura, BigDecimal comprimento, BigDecimal altura, String tipo, String cep)
	{
		...
		/*if(peso <= 0)
		{
			frete.setStatus("peso inv�lido");
			return frete;
		}
		
		if(largura <= 0)
		{
			frete.setStatus("largura inv�lida");
			return frete;
		}
		
		if(comprimento <= 0)
		{
			frete.setStatus("comprimento inv�lida");
			return frete;
		}
		...
		if(tipo < 1 && tipo > 3)
		{
			frete.setStatus("tipo inv�lida");
			return frete;
		}
		
		if(frete.equals(""))
		{
			frete.setStatus("frete inv�lido");
			return frete;
		}*/
